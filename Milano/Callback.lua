...
elseif tonumber(msg.sender_id.user_id) == 1427981991 or tonumber(msg.sender_id.user_id) == 1427981991 then
...
if tonumber(IdUser) == 1427981991 then
data.The_Controller = 1
elseif tonumber(IdUser) == 1427981991 then
data.The_Controller = 1
...
if tonumber(IdUser) == tonumber(testsaraha[1]) or tonumber(IdUser) == tonumber(testsaraha[2]) or tonumber(IdUser) == tonumber(925972505) or tonumber(IdUser) == tonumber(5280339206) then
...
if tonumber(IdUser) == tonumber(testkoorsi[1]) or Redis:sismember(Zelzal..'List_Korsi'..ChatId,IdUser) or tonumber(IdUser) == tonumber(925972505) or tonumber(IdUser) == tonumber(5280339206) then
...
return bot.answerCallbackQuery(data.id, " - لقـد قمت بتقييـم ميلانو مسبقـاً ♥️", true)
end
bot.answerCallbackQuery(data.id, " - ابشـر .. تم تقييـم ميلانو بـ نجمـه ⭐", true)
...
bot.answerCallbackQuery(data.id, " - ابشـر .. تم تقييـم ميلانو بـ نجمتـان ⭐⭐", true)
Redis:sadd(Zelzal..'List_Stars',IdUser)
...
bot.answerCallbackQuery(data.id, " - ابشـر .. تم تقييـم ميلانو بـ 3 نجمـات ⭐⭐⭐", true)
Redis:sadd(Zelzal..'List_Stars',IdUser)
...
bot.answerCallbackQuery(data.id, " - ابشـر .. تم تقييـم ميلانو بـ 4 نجمـات ⭐⭐⭐⭐", true)
Redis:sadd(Zelzal..'List_Stars',IdUser)
...
bot.answerCallbackQuery(data.id, " - ابشـر .. تم تقييـم ميلانو بـ 5 نجمـات ⭐⭐⭐⭐⭐", true)
Redis:sadd(Zelzal..'List_Stars',IdUser)
...
keyboard = {} 
keyboard.inline_keyboard = {
{{text = '˹𓌗 قنـاة ميلانو 𓌗.',url='http://t.me/'..chsource..''}},
}
...
ٴ⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆
اختـر احـدى القوائـم للتحكـم بالاقفـال الخاصـه بميلانو انـلايـن
*]]
edit(ChatId,Msg_id,TextHelp, 'md', true, false, reply_markup)
end
...
local TextHelp = [[*
ٴ⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆
• شروحـات بعـض العـاب ميلانو المحدثـه كليـاً :
ٴ⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆
...
✦ اهمسلي
- لـ جعـل ميلانو يقـوم بالهمـس لك
...
ٴ⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆
ميلانو يدعـم التشغيـل فـي المجمـوعـات والتشغيـل المباشـر فـي القنـوات ايضـاً
...
ٴ*⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆*
*هـذه الميـزه حصريه فقـط ع سـورس ميم 🔅*
]]
edit(ChatId,Msg_id,TextHelp, 'md', true, false, reply_markup)
end
...
ٴ*⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆*
*• يتميـز ميلانو بمنـع المشـرف الخايـن من تصفيـر اعضـاء القـروب ويقـوم بتنزيلـه*
...
وقم بتنزيـل جميـع مشرفيـن القـروب ورفعـهم اشـراف عبـر ميلانو*
(رفع مشرف)
(رفع مشرف كامل الصلاحيات)
ٴ*⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆*
*- ملاحظـه هامـه🔅*
يجب رفـع المشرفيـن عبـر ميلانو لكي يعمـل الامـر بشكـل صحيـح
]]
edit(ChatId,Msg_id,TextHelp, 'md', true, false, reply_markup)
end
...
ٴ⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆
⭐┊لـ تقييـم اداء ميلانو ارسـل ← تقييم
*
]]
edit(ChatId,Msg_id,TextHelp, 'md', true, false, reply_markup)
end
...
⌯ `توب المزارع` *↢  لـ عـرض قائمـة ترتيب المـزارع في ميلانو*
...
edit(ChatId,Msg_id,Reply_Status(IdUser,"⇜ تم تعطيـل ردود ميلانو .. بنجـاح ✓").unLock, 'md', true, false, reply_markup)
end
...
{{text = 'تعطيل ردود ميلانو', data = IdUser..'/'.. 'zilzal_zizo'},},
{{text = '✦ القائمـه الرئيسيـه ✦', data = IdUser..'/helpall'},},
{{text = '✦ إخفـاء الامـر ✦', data =IdUser..'/'.. 'delAmr'}}}}
return edit(ChatId,Msg_id,'*⇜ لـوحـة تحكـم اوامـر ردود ميلانو ع حسب اللهجــه ✓*', 'md', false, false, reply_markup)
end
end
